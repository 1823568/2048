...
class Game:
    ...
    # 获取分数管理器的数据
        score_mgr = self.game.score_manager
    
    # 当前分数
        score_text = font.render(f"Score: {score_mgr.get_current_score()}", True, (0, 0, 0))
        score_bg_rect = score_text.get_rect(topleft=(10, 10))
        pygame.draw.rect(self.screen, (255, 255, 255), score_bg_rect) 
        self.screen.blit(score_text, (10, 10))
    
    # 最高分
        best_text = font.render(f"Best: {score_mgr.get_highscore()}", True, (255, 200, 0))
        self.screen.blit(best_text, (10, 45))
    
    # 关卡
        level_text = font.render(f"Level: {score_mgr.get_level()}", True, (0, 200, 255))
